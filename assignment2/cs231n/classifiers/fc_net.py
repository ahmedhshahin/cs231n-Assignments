...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        affine1, aff1_cache = affine_forward(X, self.params['W1'], self.params['b1'])
        relu1, relu1_cache = relu_forward(affine1)
        scores, aff2_cache = affine_forward(relu1, self.params['W2'], self.params['b2'])
        ...
        dscores, grads['W2'], grads['b2'] = affine_backward(dloss, aff2_cache)
        drelu = relu_backward(dscores, relu1_cache)
        _, grads['W1'], grads['b1'] = affine_backward(drelu, aff1_cache)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        reg_loss = 0
        for w in range(1, self.num_layers + 1):
            reg_loss += np.sum(np.square(self.params['W{0}'.format(w)]))
